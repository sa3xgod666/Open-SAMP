	...
	if(IsActive())
	{
		if(m_pPlayerPed)
		{
			...
			{	
				...
				// UPDATE ROTATION
				m_pPlayerPed->SetTargetRotation(1.0);
			...
			if(GetState() == PLAYER_STATE_ONFOOT) {
				...
				if(m_pPlayerPed->IsInJetpackMode()) {
					m_pPlayerPed->ForceTargetRotation(1.0);
				} else {
					m_pPlayerPed->SetTargetRotation(1.0);
				}
...
bool CRemotePlayer::IsSurfingOrTurretMode()
{
	if(GetState() == PLAYER_STATE_ONFOOT) {
		if( m_ofSync.wSurfInfo != 0 &&
			...
			m_ofSync.wSurfInfo < MAX_VEHICLES ) {
			return true;
		}
	}
	return false;
...
bool CRemotePlayer::Spawn(BYTE byteTeam, int iSkin, VECTOR * vecPos, float fRotation, DWORD dwColor)
{
	...
	CPlayerPed* pPlayer = pGame->NewPlayer(m_bytePlayerID + 2, iSkin, vecPos->X,
		vecPos->Y,vecPos->Z,fRotation,pNetGame->m_iShowPlayerMarkers); // the +2 is because we can't use 0 or 1, even though the server does.*/
