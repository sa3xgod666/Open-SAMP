...
uses
  ...
  {GIFImage,} ToolWin, Classes, TlHelp32, ClipBrd, ComObj, ActiveX;
...
procedure CreateDesktopShortcut(Dest, FileName: String);
var
  IObject : IUnknown;
  ISLink : IShellLink;
  IPFile : IPersistFile;
  LinkNameW: WideString;
  CurrentPath, LinkName: String;
  Path: Array[0..MAX_PATH] Of Char;
begin
  CurrentPath:= Application.ExeName;
  ...
  with ISLink do
  begin
    SetPath(PChar(CurrentPath));
    SetWorkingDirectory(PChar(ExtractFilePath(CurrentPath)));
    SetArguments(PChar(Dest));
  end;

  SHGetSpecialFolderPath(0, Path, CSIDL_DESKTOP, False);

  LinkName:= Path;
  LinkName:= LinkName + '\' + FileName + '.lnk';
...
begin
  ...
  if not DirectoryExists(Path) then begin
    DirName:= Copy(Path, 1, FindLastOf('\', Path) - 1);
...
var
  ...
  IPAddress: String;
begin
  IPAddress:= IPList.Values[HostName];
  ...
  if GetKValue(ColorToRGB(Color)) > 16 then begin
    if GetYValue(ColorToRGB(Color)) > 16 then begin
      if GetMValue(ColorToRGB(Color)) > 16 then begin
        B:= GetMValue(ColorToRGB(Color)) - 16;
        G:= GetYValue(ColorToRGB(Color)) - 16;
        R:= GetKValue(ColorToRGB(Color)) - 16;
        Result:= RGB(R, G, B);
      end;
    end;
  end;
...
begin
  ...
  LastTopIndex:= lbServers.TopIndex;
  ...
  if (ActualIdx <> -1) and (Idx <> -1) and (Idx < Length(Servers)) then begin
    ...
    for i:= 0 to Length(Servers[Idx].aRules)-1 do begin
      NewServs.Add(IntToStr(i));
      if Servers[Idx].aRules[i].Rule = 'weburl' then begin
  ...
  if lbServers.Items.Count >= LastTopIndex then
    lbServers.TopIndex:= LastTopIndex;

  if EndUpdate = true then
...
begin
  ...
  for i:= 0 to Length(Servers)-1 do begin
    ...
    if (Servers[i].Address = StrIP) or (Servers[i].DottedAddress = StrIP) then begin
...
var
  ServFull, ServAddr, ServPort, ServPass, ServNew, URI: String;
  Str: String;
begin
  ...
  if ParamCount > 0 then begin
    ServFull:= ParamStr(1);
    URI:= Copy(ServFull, 0, 7);
    if URI = 'samp://' then begin
      ...
      Str:= 'Do you want to add ' + ServAddr + ':' + ServPort + ' to your favorites ' + #13#10 + 'or play on this server now?';
      wnd_webrunform.Label1.Caption:= Str;

      Case wnd_webrunform.ShowModal Of
        mrYes: begin
          CheckAnotherInstance;
          ServNew:= ServAddr + ':' + ServPort;
          if InputQuery('Add Server', 'Enter new server HOST:PORT...', ServNew) <> False then
            if ServNew <> '' then
              AddServer(ServNew);
        end;
        mrOk: begin
      ...
      if Pos(':', ServFull) <> 0 then begin
        ...
        /////////////////////////////////////////////////////////////
        //
        // BUG: Apperently the compiler get confused here and
        // will not add LStrLAsg() to set ServPort variable.
        //
        //  v21 = Sysutils::StrToIntDef(v27, 7777);
        //  Sysutils::IntToStr(v21, &v29);
        //  System::__linkproc__ LStrLAsg(&System::AnsiString, v29);
        //
        /////////////////////////////////////////////////////////////
        // Originally:
        //ServPort:= IntToStr(StrToIntDef(Copy(ServFull, Pos(':', ServFull)+1, 5), 7777));
        // Fixed:
        ServPort:= Copy(ServFull, Pos(':', ServFull)+1, 5);
        ServPort:= IntToStr(StrToIntDef(ServPort, 7777));
      end else begin
        ServAddr:= ServFull;
        ServPort:= '7777';
